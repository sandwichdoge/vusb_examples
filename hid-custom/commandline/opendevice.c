...
static int _shellStyleMatch(char *text, char *p) {
    ...
    for (; *p; text++, p++) {
        if (*text == 0 && *p != '*') return MATCH_ABORT;
        switch (*p) {
            case '\\':
            ...
            case '*':
                ...
                while (*text)
                    if ((matched = _shellStyleMatch(text++, p)) != MATCH_FAILED) return matched;
                return MATCH_ABORT;
            case '[':
                reverse = p[1] == '^';
                ...
                if (p[1] == ']' || p[1] == '-')
                    if (*++p == *text) matched = MATCH_SUCCESS;
                for (last = *p; *++p && *p != ']'; last = *p)
                    if (*p == '-' && p[1] != ']' ? *text <= *++p && *text >= last : *text == *p)
                        matched = MATCH_SUCCESS;
                if (matched == reverse) return MATCH_FAILED;
                continue;
        }
    }
    return *text == 0;
...
int usbGetStringAscii(usb_dev_handle *dev, int index, char *buf, int buflen) {
    ...
    if ((rval = usb_get_string_simple(dev, index, buf, buflen)) >= 0) /* use libusb version if it works */
        return rval;
    if ((rval = usb_control_msg(dev, USB_ENDPOINT_IN, USB_REQ_GET_DESCRIPTOR, (USB_DT_STRING << 8) + index, 0x0409,
                                buffer, sizeof(buffer), 5000)) < 0)
        return rval;
                        ...
                        if (len < 0) {
                            ...
                            if (shellStyleMatch(product, productNamePattern)) {
                                ...
                                if (dev->descriptor.iSerialNumber > 0) {
                                    len = usbGetStringAscii(handle, dev->descriptor.iSerialNumber, serial,
                                                            sizeof(serial));
                                }
                                if (len < 0) {
                                    errorCode = USBOPEN_ERR_ACCESS;
                                    ...
                                    if (printMatchingDevicesFp != NULL) {
                                        if (serial[0] == 0) {
                                            fprintf(printMatchingDevicesFp,
                                                    ...
                                                    dev->descriptor.idVendor, dev->descriptor.idProduct, vendor,
                                                    product);
                                        } else {
                                            fprintf(
                                                printMatchingDevicesFp,
